    ...
    if(db_user.length>0){
        ...
        if(db_user[0].email === email && db_user[0].password === pass){
            ...
            res.cookie("token", token,{
                httpOnly:true,
                expires:new Date(Date.now()+1*60),
            })
...
app.get("/aipath",(req,res)=>{
  res.render("ai page1")
})

app.get("/iotpath",(req,res)=>{
  res.render("iot page1")
})


app.get("/androidpath",(req,res)=>{
  res.render("android page1")
})

app.get("/webpath",(req,res)=>{
  res.render("website page1")
})
...
app.get("/contact", authentication,async(req, res)=>{
    res.redirect("success", {success:"You alrady loged in", name:"Hello "+req.user.name+", welcome back"})
})
